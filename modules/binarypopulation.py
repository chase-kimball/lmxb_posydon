...
class BinaryPopulation:
    ...
    def _safe_evolve(self, **kwargs):
        ...
        if optimize_ram:
            dump_rate = kwargs.get("dump_rate", int(len(indices_for_iter) / 10))
            dump_rate = np.max([dump_rate, 1])

        # Set temporary directory for population batches
        temp_directory = os.path.abspath(kwargs.get("temp_directory", "batches"))
        ...
        for j, index in enumerate(indices_for_iter):
            ...
            if(optimize_ram and j % dump_rate == 0 and j != 0 and ram_per_cpu == None):

                # Create filenames for each batch
                if(self.comm == None):
                    path = os.path.join(temp_directory, f"{j}_evolution.batch")
                else:
                    path = os.path.join(temp_directory, f"{j}_evolution.batch.{self.rank}")
            ...
            elif(ram_per_cpu != None and psutil.Process().memory_info().rss / (1024**3) >= 0.99 * ram_per_cpu):
        ...
        if((len(self.manager.binaries) != 0 or len(self.manager.history_dfs) != 0) and optimize_ram == True):

            if(self.comm == None):
                path = os.path.join(temp_directory, f"leftover_evolution.batch")
            else:
                path = os.path.join(temp_directory, f"leftover_evolution.batch.{self.rank}")
    ...
    def save(self, save_path, mode='a', **kwargs):
        ...
        if self.comm is None:
            ...
            if self.rank == 0:
                ...
                file_name = os.path.basename(absolute_filepath)
                tmp_files = [os.path.join(self.kwargs["temp_directory"], f"evolution.combined.{i}") for i in range(self.size)]
...
class PopulationManager:
    ...
    def from_hdf(self, indices, where=None, restore=False):
        ...
        for i in np.unique(hist.index):
            ...
            if bool(oneline.loc[[i]]['FAILED'].values.item()):
...
#         object_to_str = {name:'str' for i, name in enumerate(history_df.columns)
#                         if history_df.iloc[:,i].dtype == 'object'}
#         history_df = history_df.astype(object_to_str)
...
#         online_df = self.to_oneline_df(**kwargs)
#         online_df = online_df.infer_objects()
#         object_to_str = {name:'str' for i, name in enumerate(online_df.columns)
#                         if online_df.iloc[:,i].dtype == 'object'}
#         online_df = online_df.astype(object_to_str)
#         online_df = set_dtypes_oneline(online_df)
