...
path_to_Sukhbold_datasets = os.path.join(PATH_TO_POSYDON,
                                         "data/POSYDON_data/Sukhbold+16/")

path_to_Patton_datasets = os.path.join(PATH_TO_POSYDON,
                                       "data/POSYDON_data/Patton+Sukhbold20/")

path_to_Couch_datasets = os.path.join(PATH_TO_POSYDON,
                                      "data/POSYDON_data/Couch+2020/")
...
class StepSN(object):
    ...
    ECSN : stronnor, E. P. 2020, ApJ, 890, 127.
    Simulating Turbulence-aided Neutrino-dri
    ...
    def __init__(self,
        ...
        if self.mechanism in self.mechanisms:
            ...
            if self.mechanism in [
                ...
                self.path_to_Sukhbold_datasets = path_to_datasets
                ...
                # set the path to the datasets for each supernova engine
                self.path_to_Couch_datasets = path_to_datasets
                ...
                def format_data_Patton20(file_name):
                    ...
                    # Reading the dataset
                    data = np.loadtxt(
                        os.path.join(self.path_to_Patton_datasets, file_name),
    ...
    def collapse_star(self, star):
        ...
        if state in STAR_STATES_CC:
            ...
            if (self.use_interp_values
                and (star.direct is not None
                     or star.Fryer12_rapid is not None
                     or star.Fryer12_delayed is not None
                     or star.Sukhbold_16_engineN20 is not None
                     or star.Patton_Sukhbold20_engineN20 is not None)):
    ...
    def get_CO_core_params(self, star, approximation=False):
        ...
        if approximation:
            ...
            if star.state in ["H-rich_Central_C_depletion"]:
                C_core_abundance = self.C_abundance_for_H_stars(CO_core_mass)
            elif star.state_history[-1] in ["stripped_He_Central_C_depletion"]:
...
class Sukhbold16_corecollapse(object):
    ...
    def __init__(self, engine, path_engine_dataset, verbose):
        ...
        if self.engine in self.engines:
            ...
            Engine_data = read_csv(
                path_engine_dataset + "results_" + self.engine + "_table.csv"
            )
...
class Couch20_corecollapse(object):
    ...
    def __init__(self, turbulence_strength, path_engine_dataset, verbose):
        ...
        if turbulence_strength in self.turbulence_strength_options:
            ...
            Couch_data_file = open(path_to_Couch_datasets+'explDatsSTIR2.json')
